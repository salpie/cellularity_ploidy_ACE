...
calculateCellPloid <- function(copynumber_df,w,span,lowest) {
	...
	for (samp in 1:ncol(copynumber_df)) {
        ...
        if (length(copynumber_df[,samp][copynumber_df[,samp]>10]) > 10) {
            dh <- hist(copynumber_df[,samp][copynumber_df[,samp]<10],plot=FALSE, breaks=500)
        }
		y = dh$counts
		x = dh$mids
		...
		if (length(x) > 1) {
			...
			dev.off()
                ...
                samples$cellularity[samp] = 0 #set cellularity to be extremely low if there is only one peak
				samples$ploidy[samp] = 2
        }
        }
...
runACEnow <- function(readCounts=1, copyNumbersSegmented,copynumber_df,segmented_df,w=1,span,lowest=3) {
...
runACEnewploidy <- function(samples, copyNumbersSegmented,copynumber_df,segmented_df) {
	segmented_df <- as.data.frame(segmented_df)
	copynumber_df <- as.data.frame(copynumber_df)
	# Split out row names by chr, start, end
	locations_split = lapply(strsplit(rownames(copynumber_df), "-"), function(i) unlist(strsplit(i, split = ":")))
	
	rownames(copynumber_df) = gsub("Y", 24, rownames(copynumber_df))
	rownames(copynumber_df) = gsub("X", 23, rownames(copynumber_df))
...
#	if (sum(chr>22) > 1) {
#		info = readCounts@featureData@data
#		info$chr = gsub("Y", 24, info$chr)
#		info$chr = gsub("X", 23, info$chr)
#
#		chr   = as.data.frame(info$chr)
#		start = as.data.frame(info$start)
#		stop = as.data.frame(info$end)
